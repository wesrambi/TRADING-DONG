...
extern color color_fib=White;
extern color color_hud_tf=Red;
...
extern color color_shadow=White;
         ...
         if(Period() == 240)
           {
            ...
            //Variable Initialization for SnR
            lotvolumeW =                               0.03;
            stopLossW =                          8000*Point; //120 Pips 
            takeProfitW =                        3000*Point;
            snr =                                      TRUE;
           }
         //Timeframe H1
         else if(Period() == 60)
              {
               ...
               stopLossW =                       8000*Point; //100 pips
               takeProfitW =                      3000*Point; //30  pips
               snr =                                   TRUE;
              }
        //Timeframe M30
         else if(Period() == 30)
         ...
         // ((RSIValue>=70) && ((Open[1] > G_price_240) && (Close[1] > G_price_240))) || // sell ketika rsi>=70 dan harga open dan close diatas R3
          ...
          ((RSIValue>=70) && ((Open[1] > G_price_288) && (Close[1] < G_price_288)) || ((RSIValue>=70) && (Open[1] < G_price_288) && (Close[1] > G_price_288)))  // 288=AVER
            ...
            // Jika harga close sebelumnya lebih tinggi dari S1
           {
            // Buka posisi sell
            sinyal="sell";
            int ticket2 = OrderSend(Symbol(), OP_SELL, lotvolumeW, Bid, 300, Ask + stopLossW, Ask - takeProfitW, "SnR Sell order D1", 0, 0, Red);
            if(ticket2 > 0)
              {
            ...
            for(j=i;j>=0;j--) //Memeriksa apakah harga sudah mencapai SND atau belum
              {
               ...
               if(exit && iHigh(pair,tf,j)>p2) //Jika iHigh melebihi harga SND, area SND akan diwarnai ulang
                 {
                  retouch=true;
                  if(zone_fibs && fib_sup_hit==0){ fib_sup=p2; fib_sup_hit=j;} //Menghitung fibo?
                  break;
                 }
              }
           }
            ...
            if(draw)
              {
               ...
               //for(int ots=OrdersTotal()-1; ots>=0; ots--)
               //   {
               //    if(OrderSelect(ots,SELECT_BY_POS,MODE_TRADES)==true)
               //      {
               //       if((OrderType()==OP_SELLLIMIT)<1)
               //         {
               //          if(c==color_dem_strong && iMA(Symbol(),0,14,0,MODE_EMA,PRICE_MEDIAN,0) >= iMA(Symbol(),0,14,0,MODE_EMA,PRICE_MEDIAN,10))
               //            {
               //             OrderSend(Symbol(),OP_SELLLIMIT,0.01,p2,30,6000*Point+BuferDn1[i],(p2-10000*Point),"POS_SUP",MagicNumber1+1,TimeCurrent()+18000,Red);
               //            }
               //         }
               //        else if(iMA(Symbol(),0,14,0,MODE_EMA,PRICE_MEDIAN,0) >= iMA(Symbol(),0,14,0,MODE_EMA,PRICE_MEDIAN,10))
               //         {
               //          OrderSend(Symbol(),OP_SELLLIMIT,0.01,p2,30,6000*Point+BuferDn1[i],(p2-10000*Point),"POS_SUP",MagicNumber1+2,TimeCurrent()+18000,Red);
               //         }
               //      }
               //   }
               
               if(OrdersTotal()<1 && c==color_sup_strong)
                  {
                   if(iMA(Symbol(),0,14,0,MODE_EMA,PRICE_MEDIAN,0) >= iMA(Symbol(),0,14,0,MODE_EMA,PRICE_MEDIAN,10))
                     {
                      OrderSend(Symbol(),OP_SELLLIMIT,0.01,p2,30,BuferDn1[i]+6000*Point,(p2-10000*Point),"POS_SUP",MagicNumber1,TimeCurrent()+18000,Red);
                     }
                  }
               
               //if(OrdersTotal()<1 && (c==color_sup_weak || c==color_sup_retouch))
               //   {
               //    OrderSend(Symbol(),OP_BUYLIMIT,0.01,p2+5000*Point,30,(p2+5000*Point)-5000*Point,(p2+5000*Point)+7000*Point,"B_SUP_WEAK",MagicNumber1+1,TimeCurrent()+18000,Green);
               //   }
               ...
               if(globals) //Menyimpan variabel ke nilai global dari area SND yang sudah terjadi
                 {
            ...
            if(draw)
              {
               ...
               //for(int otd=OrdersTotal()-1; otd>=0; otd--)
               //   {
               //    if(OrderSelect(otd,SELECT_BY_POS,MODE_TRADES)==true)
               //      {
               //       if((OrderType()==OP_BUYLIMIT) && (OrderComment()=="POB_DEM"))
               //         {
               //          if(c==color_dem_strong && iMA(Symbol(),0,14,0,MODE_EMA,PRICE_MEDIAN,0) <= iMA(Symbol(),0,14,0,MODE_EMA,PRICE_MEDIAN,10))
               //            {
               //             OrderSend(Symbol(),OP_BUYLIMIT,0.01,p2,30,BuferUp1[i]-6000*Point,(p2+10000*Point),"POB_DEM",122,TimeCurrent()+18000,Green);
               //            }
               //         }
               //        else if(iMA(Symbol(),0,14,0,MODE_EMA,PRICE_MEDIAN,0) <= iMA(Symbol(),0,14,0,MODE_EMA,PRICE_MEDIAN,10))
               //         {
               ...
               //if(OrdersTotal()<1 && c==color_dem_strong)
               //   {
               //    if(iMA(Symbol(),0,14,0,MODE_EMA,PRICE_MEDIAN,0) <= iMA(Symbol(),0,14,0,MODE_EMA,PRICE_MEDIAN,10))
               //      {
               //       OrderSend(Symbol(),OP_BUYLIMIT,0.01,p2,30,BuferUp1[i]-9000*Point,(p2+5000*Point),"POB_DEM",MagicNumber2,TimeCurrent()+18000,Green);
               //      }
               //   }
               
               //if(OrdersTotal()<1 && (c==color_dem_weak || c==color_dem_retouch))
               //   {
               //    OrderSend(Symbol(),OP_SELLLIMIT,0.01,BuferUp1[i]+3000*Point,30,(BuferUp1[i]+3000*Point)+10000*Point,(BuferUp1[i]+3000*Point)-8000*Point,"S_DEM_WEAK",MagicNumber2+1,TimeCurrent()+18000,Red);
               //   }
